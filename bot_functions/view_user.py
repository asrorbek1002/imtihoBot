...
def start_view_user(update, context):
    ...
    if users:
        ...
        for index, user in enumerate(users, start=1):
            user_id, first_name, last_name, *_ = user
            message = f"<b>{index}. Foydalanuvchi: <i>{first_name} {last_name}</i>\n📎U haqida: <a href='https://t.me/{bot_username}?start=tg_{user_id}'>LINK</a></b>"
            messages.append(message)
        
        reply_text = "\n\n".join(messages)
    else:
        reply_text = "Hech qanday foydalanuvchi topilmadi."
    
    query.edit_message_text(reply_text, parse_mode="HTML")
        ...
        if user_id.isdigit():
            ...
            if user_info:
                ...
                message = (
                    ...
                    f"Foydalanuvchini bazadan o'chirish uchun <a href='https://t.me/{bot_username}?start=del_{user_id}'>bosing</a>"
                )
            else:
